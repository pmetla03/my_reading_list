...
const alertMessage = ref('Book updated successfully!')
...
async function handleSubmit({ data }) {
  ...
  const bookData = {
    title: form.title,
    author: form.author,
    isReady: is_read.value
  };
  const { data: response, error } = await useFetch(`/api/books/update/${bookId}`, {
  method: 'PATCH',
...
async function loadBookDetails() {
  try {
    ...
    const { data, error } = await useFetch(`/api/books/book/${bookId}`, {
      method: 'GET',
      headers: {
    ...
    const bookResponse = JSON.parse(JSON.stringify(data.value))
    if (error.value) {
    ...
    if (!bookResponse.success || !bookResponse.data) {
     ...
     form.title = bookResponse.data.title;
     form.author = bookResponse.data.author;
     is_read.value=bookResponse.data.is_read
